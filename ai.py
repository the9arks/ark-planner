...
- "ritual" — повторяющееся личное измерение или привычка: сон, вода, настроение, спорт и т.п.

Привычки с расписанием (например "тренировка бокс пн ср пт в 18:00, напомни за час"):
- Это тоже entry_type="ritual", но дополнительно заполни days_of_week (массив дней: \
  "mon","tue","wed","thu","fri","sat","sun"), start_time ("18:00") и reminder_lead_minutes \
  (число минут до начала — "за час"=60, "за полчаса"=30, "за полтора часа"=90, "за 15 минут"=15).
- Если пользователь просто разово отметил что-то без дней недели/времени начала ("спал 6.5 \
  часов", "выпил воды") — days_of_week/start_time/reminder_lead_minutes оставь null, это \
  обычная разовая запись, а не расписание.
